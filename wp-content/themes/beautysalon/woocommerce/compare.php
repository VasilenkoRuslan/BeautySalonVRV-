...
<h1>
	...
	if (!$is_iframe) :
		?>
		<a class="close"
		   href="#"><?php esc_html_e('Close window [X]', 'yith-woocommerce-compare'); ?></a><?php endif; ?>
</h1>
...
<div id="yith-woocompare" class="woocommerce">
	...
	<table class="compare-list table table-hover" cellpadding="0" cellspacing="0" <?php if (empty($products)) {
		...
		<?php else : ?>
			...
			<?php foreach ($fields as $field => $name) : ?>
				...
				if (
					...
					foreach ($products as $product) {
						$product_html_value = $product->fields[$field];

						if (
								(empty($product_html_value)) ||
								($product_html_value == '<span>N/A</span>') ||
								($product_html_value == '<span>-</span>')
						) {
							$count_empty_values++;
						}
					}

					if ($count_empty_values == $total_products) {
						//$is_empty_line = true;
						continue;
					}
				}
